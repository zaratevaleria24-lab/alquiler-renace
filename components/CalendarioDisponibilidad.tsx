...
export default function CalendarioDisponibilidad({
  ...
  const elegir = (d: string) => {
    ...
    // Toque antes de la llegada: se corrige la llegada, no es una salida.
    if (d <= llegada) {
      setLlegada(d);
      return;
    }
    ...
    <div className="mt-5 border-t border-line pt-5">
      ...
      <div className="mt-3 grid grid-cols-7 text-center">
        ...
        {Array.from({ length: totalDias }, (_, i) => {
          const d = `${mesVista}-${String(i + 1).padStart(2, '0')}`;
          const pasado = d < hoy;
          ...
          return (
            <button
              ...
              className={`mx-auto flex h-9 w-9 items-center justify-center rounded-full text-meta tabular-nums transition-colors ${
                esLlegada || esSalida
      ...
      <p className="mt-2.5 text-[12px] leading-relaxed text-ink-subtle">
        Los días tachados ya están tomados.

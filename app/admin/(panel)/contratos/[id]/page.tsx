...
import { datosDe, getContrato, urlContrato } from '@/lib/contratos';
    ...
    <div>
      ...
      <Tarjeta className="mt-6 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {abierto && wa && <form action={marcarEnviadoAction}><input type="hidden" name="id" value={c.id} /><a href={wa} target="_blank" rel="noopener" className="inline-flex min-h-[46px] items-center gap-2 rounded-control border border-line bg-white px-4 text-meta font-medium text-brand-deep hover:border-brand/40" onClick={undefined}><MessageCircle className="h-4 w-4" />Mandar por WhatsApp</a></form>}
        ...
        {c.estado === 'firmado' && <p className="mt-4 text-ui text-ink-muted">Firmado el {new Date(c.firmadoAt!).toLocaleString('es-VE', { timeZone: 'America/Caracas' })} por {c.firmaNombre} ({c.firmaDocumento}). Verificación <span className="mono-data">{c.firmaHash.slice(0, 32)}</span>. Navegador: {c.firmaAgente.slice(0, 80)}</p>}
      </Tarjeta>

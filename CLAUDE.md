...
Sections and their docs: `SEO.md`, `VENTAS.md` (En venta), `GUIA.md` (tourist guide, `/guia`), `CALENDARIO.md`, `ENLACES.md`, `CONTRATOS.md` (guest contracts with e-signature, `/admin/contratos`), `CRM.md` (welcome coupon + contacts CRM, `/admin/contactos`), `AIRBNB-CORREO.md` (plan: Airbnb notification emails as an event source, calendar congruence), `SEGURIDAD.md` (security audit and rules — the app connects as the least-privilege role `margarita_app`; migrations use `margarita`).
